...
sap.ui.controller("sap.ui.demo.myFiori.view.App", {
	...
	navHandler: function (channelId, eventId, data) {
		if (eventId === "to") {
			...
			console.log("'nav' event cannot be processed. There's no handler registered for event with id: " + eventId);
		}
	},
	...
	navTo : function (id, data, writeHistory) {
		if (id === undefined) {
			...
			console.log("navTo failed due to missing id");
	...
	navBack : function (id) {
		if (!id) {
			...
			console.log("navBack - parameters id must be given");
			...
			if (sap.m.InstanceManager.hasOpenDialog()) {
				...
				console.log("navBack - closed dialog(s)");
			}
			...
			if (currentId !== id) {
				...
				console.log("navBack - back to page: " + id);
			}
		}
	}	
